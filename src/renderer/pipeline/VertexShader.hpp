...
namespace ST {
	...
	struct VertexOut {
		...
		VertexOut() : color(0,0,0,1), position(0,0,0,1), worldPosition(0,0,0), normal(0,0,1), tangent(1,0,0), tangentSign(1.0f), texCoord(0,0) {
	...
	public:
		...
		static VertexOut transformVertex(const Vertex& vertex, const Uniform& uni) {
			...
			out.tangentSign = vertex.tangentSign;
	...
	inline VertexOut lerpVertexOut(const VertexOut& a, const VertexOut& b, float t) {
		...
		r.tangentSign   = a.tangentSign   + (b.tangentSign   - a.tangentSign)   * t;
