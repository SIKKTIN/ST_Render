...
namespace ST {
	...
	public:
		...
		Color& operator+=(const Color& other) {
			...
			// Alpha is NOT accumulated: it represents opacity / premultiplication
			// factor, and adding two opaque colours (a=1 each) should not
			// produce a double-opaque colour (a=2).  The RGB channels are what
			// get mixed; alpha stays at the left-operand's value.
			return *this;
		}
