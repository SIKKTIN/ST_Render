...
void TestModule_2D_Scene::renderScene(ST::FrameBuffer* fb, int renderW, int renderH) {
    ...
    // Find CinemachineTarget and bind it to Cinemachine
    ST::Cinemachine* mainCam = m_scene.findCinemachine();

    ST::Matrix4x4 view, projection;

    if (m_viewportMode == 0) {
        // Scene mode: clear first, then render
    ...
    if (m_selected && m_selected->showAABB) {
    ...
    if (m_selected && m_viewportMode == 0) {
        ST::Matrix4x4 model = buildWorldMatrix(m_selected);
    ...
    if (m_showGrid && m_viewportMode == 0) {
...
bool TestModule_2D_Scene::renderControls() {
    ...
    bool changed = ImGui::Combo("Anti-Aliasing", &m_aaMode, "None\0SSAA (2x)\0FXAA\0");
...
void TestModule_2D_Scene::renderCreatePanel() {
    ...
    const char* typeNames[] = { "Quad", "Circle", "Sprite2D", "MusicPlayer", "CinemachineTarget" };
    static int curType = 0;
    ImGui::Combo("Type", &curType, typeNames, 5);
